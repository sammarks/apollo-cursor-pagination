  ...
  return (nodesAccessor, cursorOfInitialNode, {
    ...
    const executeFilterQuery = query => operateOverScalarOrArray(query, orderColumn, (orderBy, index, prev) => {
      ...
      if (index > 0) {
        ...
        const nested = prev[operation](`(${formatColumnIfAvailable(orderColumn[index - 1], formatColumnFn)} = ? and ${formatColumnIfAvailable(orderBy, formatColumnFn)} ${comparator} ?)`, [values[index - 1], values[index]]);
        return nested;
      }
      ...
      if (lastValue === null || lastValue === undefined) {
        return prev[operation](`(${formatColumnIfAvailable('id', formatColumnFn)} ${comparator} ?) or (${formatColumnIfAvailable(lastOrderColumn, formatColumnFn)} IS NOT NULL)`, [id]);
      }

      return prev[operation](`(${formatColumnIfAvailable(lastOrderColumn, formatColumnFn)} = ? and ${formatColumnIfAvailable('id', formatColumnFn)} ${comparator} ?)`, [lastValue, id]);
    });
...
const orderNodesBy = (nodesAccessor, {
  ...
  }, (prev, isArray) => isArray ? prev.orderBy(formatColumnIfAvailable('id', formatColumnFn), ascOrDesc[0]) : prev.orderBy(formatColumnIfAvailable('id', formatColumnFn), ascOrDesc));
  return result;
